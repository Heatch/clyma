...
const DEFAULT_MARKET_COUNT = 24
const MAX_MARKET_COUNT = 40
const parsedCount = Number.parseInt(process.env.GEMINI_MARKET_COUNT ?? "", 10)
const MARKET_COUNT =
  Number.isFinite(parsedCount) && parsedCount > 0
    ? Math.min(parsedCount, MAX_MARKET_COUNT)
...
function trendTo(target: number): number[] {
  ...
  return Array.from({ length: HISTORY_POINT_COUNT }, (_, index) =>
    round(
      clamp(
        start + (target - start) * (index / (HISTORY_POINT_COUNT - 1)),
        0.02,
        0.98,
      ),
      4,
...
function mapMarket(market: GeminiMarket, index: number): ClimateMarket | null {
  ...
  const createdAt = new Date(
    now - (HISTORY_POINT_COUNT - 1) * HISTORY_CADENCE_DAYS * DAY_MS,
  ).toISOString()
  ...
  const climateMarket: ClimateMarket = {
    ...
    history: buildHistory({
      ...
      cadenceDays: HISTORY_CADENCE_DAYS,
    }),
    evidence,
